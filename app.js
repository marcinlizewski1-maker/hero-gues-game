(function () {
  ...
  let multiplayerSocketPromise = null;
  let multiplayerSocket = null;
  let multiplayerKeyboardBound = false;
  ...
  async function kickUserFromRoomAsAdmin(token, userId) {
    return requestJson("/admin/kick-room/" + userId, {
      method: "POST",
      headers: getAuthHeaders(token)
    });
  }
  ...
  async function createMultiplayerRoom(token) {
    return requestJson("/multiplayer/create-room", {
      method: "POST",
      headers: getAuthHeaders(token)
    });
  }

  async function joinMultiplayerRoom(token, roomCode) {
    return requestJson("/multiplayer/join-room", {
      method: "POST",
      headers: getAuthHeaders(token),
  ...
  function loadSocketClient() {
    ...
    socketScriptPromise = new Promise(function (resolve, reject) {
      ...
      script.onerror = function () {
        reject(new Error("Nie udalo sie zaladowac klienta multiplayer."));
      };
  ...
  function disconnectMultiplayerSocket() {
    ...
    multiplayerSocketPromise = null;
  }

  function updateMultiplayerTimerDisplay(secondsLeft) {
    const timerNode = document.querySelector("#multiplayerTimerValue");

    if (timerNode) {
      timerNode.textContent = (typeof secondsLeft === "number" ? secondsLeft : 0) + " s";
    }
  }

  function bindMultiplayerKeyboard() {
    if (multiplayerKeyboardBound) {
      return;
    }
    ...
    document.addEventListener("keydown", function (event) {
      ...
      const usedLetters = hangman && Array.isArray(hangman.usedLetters)
        ? hangman.usedLetters
        : hangman && Array.isArray(hangman.guessedLetters)
      ...
      handleMultiplayerAnswer(letter);
    });

    multiplayerKeyboardBound = true;
  }

  async function connectMultiplayerSocket(token) {
    if (multiplayerSocket && multiplayerSocket.connected) {
      return multiplayerSocket;
    }

    if (multiplayerSocketPromise) {
      return multiplayerSocketPromise;
    }

    multiplayerSocketPromise = loadSocketClient().then(function (ioFactory) {
  ...
  function bindMultiplayerSocketEvents(socket) {
    socket.off("multiplayer:state");
    socket.off("multiplayer:turn-start");
    socket.off("multiplayer:timer-update");
    socket.off("multiplayer:turn-result");
    socket.off("multiplayer:finished");
    socket.off("gameOver");
    ...
    socket.on("multiplayer:turn-start", function (payload) {
      ...
      updateMultiplayerTimerDisplay(typeof payload.secondsLeft === "number" ? payload.secondsLeft : current.secondsLeft || 0);
      store.setState({
        ...
        multiplayer: Object.assign({}, current, {
          ...
          question: null,
          roundEndsAt: null,
          secondsLeft: typeof payload.secondsLeft === "number" ? payload.secondsLeft : current.secondsLeft || 0,
    ...
    socket.on("multiplayer:turn-result", function (payload) {
      ...
      store.setState({
        multiplayer: Object.assign({}, current, {
          message: payload && payload.message ? payload.message : (payload.timeout ? "Timeout ruchu." : "Ruch zakonczony."),
          selectedOptionId: ""
        })
      });
    });
    ...
    socket.on("multiplayer:finished", function (roomState) {
      ...
      store.setState({
        ...
        multiplayer: Object.assign({}, store.getState().multiplayer, {
          ...
          roomCode: roomState.code,
          players: roomState.players || [],
          me: roomState.me || store.getState().multiplayer.me || null,
          opponent: roomState.opponent || null,
          hangman: roomState.hangman || store.getState().multiplayer.hangman || null,
          question: null,
          result: result,
          roundEndsAt: null,
          secondsLeft: 0,
          ...
          message: result.reason === "disconnect"
            ...
            : isWinner
              ...
              : result.winnerId
                ? "Mecz zakonczony. Przeciwnik wygral."
                : "Mecz zakonczony remisem."
        })
      });
    });
    ...
    socket.on("multiplayer:error", function (payload) {
      ...
      store.setState({
        multiplayer: Object.assign({}, current, {
          message: payload && payload.error ? payload.error : "Blad multiplayera.",
          status: "error"
        })
      });
    });

    socket.on("multiplayer:kicked", function (payload) {
      disconnectMultiplayerSocket();
      store.setState({
        activeView: "menu",
        multiplayer: {
          status: "idle",
          message: payload && payload.message ? payload.message : "Zostales wyrzucony z pokoju przez administratora.",
          roomCode: "",
          players: [],
          me: null,
          opponent: null,
        ...
        authStatus: "error",
        authMessage: payload && payload.message ? payload.message : "Administrator wyrzucil gracza z pokoju."
      });
    });
    ...
    socket.on("disconnect", function () {
      ...
      store.setState({
        multiplayer: Object.assign({}, current, {
          status: "disconnected",
          message: "Polaczenie realtime zostalo przerwane."
        })
      });
    });
  }
  ...
  async function loadSuperheroes() {
    ...
    if (heroesLoadPromise) {
      return heroesLoadPromise;
    }

    heroesLoadPromise = requestJson("/superheroes")
      .then(function (allHeroes) {
  ...
  function renderHomeSection(container, state, handlers, dataStatus) {
    const loadingBanner = dataStatus === "loading"
      ? '<div class="placeholder-menu compact-loading"><div><div class="mode-badge">Loading</div><h2 class="mode-title">Ladowanie bohaterow...</h2><p>Baza Marvel/DC przygotowuje sie w tle. Tryby odblokuja sie po chwili.</p></div></div>'
      : "";
    const userLabel = state.currentUser
      ? "Zalogowano jako " + state.currentUser.nickname + ". Wybierz zakladke i wskakuj do gry."
      : "Przelaczaj sie miedzy zakladkami i uruchamiaj funkcje bez szukania ich w dlugim widoku.";

    container.innerHTML = '<div class="content-grid nav-section-screen">' +
      '<section class="start-hero-card nav-hero-card">' +
        ...
        '<p class="mode-description">' + userLabel + '</p>' +
        loadingBanner +
        '<div class="start-actions">' +
          '<button class="action-button start-game-button" type="button" id="homeStartGameButton">Start Game</button>' +
          '<button class="action-button secondary-button" type="button" id="homeOpenRankingButton">Leaderboard</button>' +
        '</div>' +
      '</section>' +
      '<section class="feature-grid quick-nav-grid">' +
        '<article class="feature-card quick-nav-card" data-home-nav="game"><h3>Gra</h3><p class="mode-meta">Classic, Fight, Power, Lore i Daily w jednym miejscu.</p></article>' +
        '<article class="feature-card quick-nav-card" data-home-nav="multiplayer"><h3>Multiplayer</h3><p class="mode-meta">Pokoje 1v1 i wisielec w czasie rzeczywistym.</p></article>' +
        '<article class="feature-card quick-nav-card" data-home-nav="ranking"><h3>Ranking</h3><p class="mode-meta">Top gracze, punkty i streak bez przewijania calej strony.</p></article>' +
        '<article class="feature-card quick-nav-card" data-home-nav="profile"><h3>Profil</h3><p class="mode-meta">Logowanie, rejestracja i panel admina w osobnej zakladce.</p></article>' +
      '</section>' +
    '</div>';

    container.querySelector("#homeStartGameButton").addEventListener("click", handlers.onStart);
    container.querySelector("#homeOpenRankingButton").addEventListener("click", handlers.onLeaderboardOpen);
    container.querySelectorAll("[data-home-nav]").forEach(function (card) {
      card.addEventListener("click", function () {
        handlers.onNavigate(card.dataset.homeNav);
      });
    });
  }

  function renderGameSection(container, leaderboard, progress, handlers, dataStatus) {
    const loadingBanner = dataStatus === "loading"
      ? '<div class="placeholder-menu compact-loading"><div><div class="mode-badge">Loading</div><h2 class="mode-title">Ladowanie bohaterow...</h2><p>Baza Marvel/DC przygotowuje sie w tle. Tryby odblokuja sie po chwili.</p></div></div>'
      : "";
    ...
    container.innerHTML = '<div class="content-grid nav-section-screen">' +
      '<div class="content-header">' +
        '<div><div class="mode-badge">Gra</div><h2 class="mode-title">Wybierz tryb i zacznij runde</h2></div>' +
        '<button class="action-button" type="button" id="gameStartClassicButton">Start Classic</button>' +
      '</div>' +
      '<p class="mode-description">Lewy panel zawiera wszystkie tryby gry. Wybierz jeden z nich bez przechodzenia przez inne sekcje aplikacji.</p>' +
      loadingBanner +
      '<section class="status-strip">' + createStats([
        { title: "Tryby", value: "5 aktywnych" },
        { title: "Streak", value: String((progress && progress.streak) || 0) },
        { title: "Punkty", value: String((progress && progress.points) || 0) }
      ]) + '</section>' +
      renderInGameLeaderboard(leaderboard) +
    '</div>';

    container.querySelector("#gameStartClassicButton").addEventListener("click", handlers.onStart);
  }

  function renderRankingSection(container, leaderboard, handlers) {
    container.innerHTML = '<div class="content-grid nav-section-screen">' +
      '<div class="content-header">' +
        '<div><div class="mode-badge">Ranking</div><h2 class="mode-title">Top Players</h2></div>' +
        '<button class="action-button" type="button" id="rankingPlayButton">Przejdz do gry</button>' +
      '</div>' +
      '<p class="mode-description">Najlepsze wyniki sa zawsze pod reka, bez przewijania przez pozostale funkcje.</p>' +
      renderLeaderboardBlock(leaderboard) +
    '</div>';

    container.querySelector("#rankingPlayButton").addEventListener("click", handlers.onStart);
  }
  ...
  function renderAdminSectionEnhanced(adminUsers, selectedAdminUsers) {
    ...
    return '<section class="admin-panel"><div class="admin-panel-head"><div><div class="mode-badge">Admin</div><h3>Panel admina</h3></div><button class="action-button secondary-button" type="button" id="refreshAdminUsersButton">Odswiez</button></div><section class="admin-bulk-panel"><label class="admin-select-all"><input type="checkbox" id="selectAllAdminUsers" ' + (allSelected ? "checked" : "") + '>Zaznacz wszystkich</label><p class="mode-meta">Zaznaczono: ' + selectedIds.length + ' graczy</p><div class="admin-bulk-actions"><button class="action-button admin-small-button" type="button" id="bulkAddPointsButton" ' + (!selectedIds.length ? "disabled" : "") + '>+50 wszystkim</button><button class="action-button secondary-button admin-small-button" type="button" id="bulkRemovePointsButton" ' + (!selectedIds.length ? "disabled" : "") + '>-50 wszystkim</button><button class="action-button admin-ban-button" type="button" id="bulkBanButton" ' + (!selectedIds.length ? "disabled" : "") + '>Ban zaznaczonych</button><button class="action-button secondary-button admin-small-button" type="button" id="bulkUnbanButton" ' + (!selectedIds.length ? "disabled" : "") + '>Odbanuj zaznaczonych</button><button class="action-button admin-delete-button" type="button" id="bulkDeleteUsersButton" ' + (!selectedIds.length ? "disabled" : "") + '>Usun zaznaczonych</button></div></section><div class="admin-users-list">' +
      adminUsers.map(function (user) {
        const userId = String(user.id || user._id);
        return '<article class="admin-user-row"><div class="admin-user-main"><label class="admin-user-selector"><input type="checkbox" data-admin-select="' + userId + '" ' + (selectedIds.indexOf(userId) !== -1 ? "checked" : "") + '><span>Zaznacz</span></label><div><strong>' + user.nickname + '</strong><p>' + user.email + ' • ' + user.role + (user.banned ? ' • zbanowany' : "") + (user.activeRoomCode ? ' • pokoj ' + user.activeRoomCode : '') + '</p></div></div><div class="admin-user-stats"><span>' + user.points + ' pkt</span><span>streak ' + user.streak + '</span></div><div class="admin-user-actions"><button class="action-button admin-small-button" type="button" data-admin-points="' + userId + '" data-delta="50">+50</button><button class="action-button secondary-button admin-small-button" type="button" data-admin-points="' + userId + '" data-delta="-50">-50</button><button class="action-button admin-ban-button" type="button" data-admin-ban="' + userId + '" data-banned="' + (!user.banned) + '">' + (user.banned ? "Odbanuj" : "Ban") + '</button>' + (user.activeRoomCode ? '<button class="action-button secondary-button admin-small-button" type="button" data-admin-kick-room="' + userId + '">Wyrzuc z pokoju</button>' : '') + '<button class="action-button admin-delete-button" type="button" data-admin-delete="' + userId + '">Usun</button></div></article>';
      }).join("") +
    '</div></section>';
  }

  function renderMultiplayerPanel(multiplayerState, currentUser) {
    const state = multiplayerState || {};
    const disabled = !currentUser ? "disabled" : "";
    const statusLine = state.message
      ? '<p class="mode-meta multiplayer-copy">' + state.message + '</p>'
      : '<p class="mode-meta multiplayer-copy">Gra 1v1 w czasie rzeczywistym. Zaloguj sie, stworz pokoj albo dolacz po kodzie.</p>';

    return '<section class="multiplayer-panel">' +
      '<div class="auth-panel-header"><div><div class="mode-badge">Multiplayer</div><h3>Pojedynek 1v1</h3></div><p class="mode-meta">Socket.IO + JWT</p></div>' +
      statusLine +
      '<div class="multiplayer-room-actions">' +
        '<button class="action-button" type="button" id="createRoomButton" ' + disabled + '>Stworz pokoj</button>' +
        '<form class="multiplayer-join-form" id="joinRoomForm">' +
          '<input class="guess-input" id="joinRoomCodeInput" name="roomCode" maxlength="4" placeholder="Kod pokoju" ' + disabled + '>' +
          '<button class="action-button secondary-button" type="submit" ' + disabled + '>Dolacz</button>' +
        '</form>' +
      '</div>' +
      '<div class="multiplayer-room-meta">' +
        '<article class="status-card"><h3>Pokoj</h3><p class="mode-meta">' + (state.roomCode || "---") + '</p></article>' +
        '<article class="status-card"><h3>Status</h3><p class="mode-meta">' + (state.status || "idle") + '</p></article>' +
      '</div>' +
    '</section>';
  }
  ...
  function renderMultiplayerView(container, multiplayerState, currentUser, handlers) {
    ...
    const letterButtonsView = hangmanView
      ? "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map(function (letter) {
          const isSelected = state.selectedOptionId === letter;
          const isUsed = guessedLettersView.indexOf(letter) !== -1;
          return '<button class="multiplayer-key ' + (isSelected ? "is-active " : "") + (isUsed ? "is-used" : "") + '" type="button" data-multiplayer-letter="' + letter + '" ' + (!isMyTurnView || isUsed || gameOverView ? "disabled" : "") + ' aria-label="Litera ' + letter + '">' + letter + '</button>';
        }).join("")
      : "";

    const gameOverOverlayView = gameOverView && gameOverPayloadView
    ...
    container.innerHTML = '<div class="content-grid">' +
      '<div class="content-header">' +
        ...
        '<button class="action-button" type="button" id="exitMultiplayerButton">Back to menu</button>' +
      '</div>' +
      renderFeedbackBanner(state.message, state.status === "finished" ? (state.result && state.result.winnerId && state.me && state.result.winnerId === state.me.id ? "won" : "lost") : "playing") +
    ...
    container.querySelector("#exitMultiplayerButton").addEventListener("click", handlers.onBack);
    container.querySelectorAll("[data-multiplayer-letter]").forEach(function (button) {
      button.addEventListener("click", function () {
        handlers.onAnswer(button.getAttribute("data-multiplayer-letter"));
      });
    });
    const playAgainButton = container.querySelector("#playAgainButton");
    if (playAgainButton) {
      playAgainButton.addEventListener("click", handlers.onPlayAgain);
    }
    ...
    const me = state.players && state.players[0]
      ? state.players.find(function (player) {
          return state.me && player.id === state.me.id;
        }) || null
      : null;
    const playerCards = (state.players || []).map(function (player) {
      const isMe = state.me && player.id === state.me.id;
      return '<article class="status-card multiplayer-score-card"><h3>' + (isMe ? player.nickname + " (Ty)" : player.nickname) + '</h3><p class="mode-meta">' + player.score + ' pkt</p><span class="' + (player.connected ? 'multiplayer-connected' : 'multiplayer-disconnected') + '">' + (player.connected ? 'online' : 'offline') + '</span></article>';
    }).join("");
    const optionButtons = question
      ? question.options.map(function (option) {
          const isSelected = state.selectedOptionId === option.id;
          return '<button class="mode-card multiplayer-option ' + (isSelected ? "is-active" : "") + '" type="button" data-multiplayer-answer="' + option.id + '" ' + (state.status !== "playing" ? "disabled" : "") + '><span class="mode-card-title">' + option.name + '</span><span class="mode-card-copy">Wybierz jako odpowiedz</span></button>';
        }).join("")
      : "";
    ...
    container.innerHTML = '<div class="content-grid">' +
      ...
      '<section class="classic-panel">' +
        '<div class="classic-status">' +
          '<article class="status-card"><h3>Przeciwnik</h3><p class="mode-meta">' + (state.opponent ? state.opponent.nickname : "Czekamy...") + '</p></article>' +
          '<article class="status-card"><h3>Runda</h3><p class="mode-meta">' + (question ? question.round + " / " + question.totalRounds : "-") + '</p></article>' +
          '<article class="status-card"><h3>Timer</h3><p class="mode-meta">' + (typeof state.secondsLeft === "number" ? state.secondsLeft + " s" : "-") + '</p></article>' +
        '</div>' +
        '<div class="multiplayer-score-grid">' + playerCards + '</div>' +
        (question
          ? '<div class="multiplayer-question-card"><h3>Wspolne pytanie dla obu graczy</h3><p class="mode-description">Publisher: ' + question.clue.publisher + ' • alignment: ' + question.clue.alignment + '</p><div class="multiplayer-stats-grid"><article class="status-card"><h3>Intelligence</h3><p class="mode-meta">' + question.clue.stats.intelligence + '</p></article><article class="status-card"><h3>Strength</h3><p class="mode-meta">' + question.clue.stats.strength + '</p></article><article class="status-card"><h3>Speed</h3><p class="mode-meta">' + question.clue.stats.speed + '</p></article><article class="status-card"><h3>Durability</h3><p class="mode-meta">' + question.clue.stats.durability + '</p></article><article class="status-card"><h3>Combat</h3><p class="mode-meta">' + question.clue.stats.combat + '</p></article></div><div class="multiplayer-options-grid">' + optionButtons + '</div></div>'
          : '<div class="placeholder-menu compact-loading"><div><div class="mode-badge">Lobby</div><h2 class="mode-title">Czekamy na start meczu</h2><p>Gdy dwoch graczy bedzie online w pokoju, serwer automatycznie rozpocznie runde.</p></div></div>') +
      '</section>' +
    '</div>';

    container.querySelector("#exitMultiplayerButton").addEventListener("click", handlers.onBack);
    container.querySelectorAll("[data-multiplayer-answer]").forEach(function (button) {
      button.addEventListener("click", function () {
        handlers.onAnswer(button.getAttribute("data-multiplayer-answer"));
      });
    });
  }

  function renderAppNavbar(navElement, activeSection, onNavigate) {
    if (!navElement) {
      return;
    }

    navElement.innerHTML = APP_NAV_ITEMS.map(function (item) {
      return '<button class="app-nav-button ' + (activeSection === item.id ? "is-active" : "") + '" type="button" data-nav-id="' + item.id + '">' + item.label + "</button>";
    }).join("");

    navElement.querySelectorAll("[data-nav-id]").forEach(function (button) {
      button.addEventListener("click", function () {
        onNavigate(button.dataset.navId);
      });
    });
  }

  function renderProfileSection(container, authState, handlers) {
    container.innerHTML = '<div class="content-grid nav-section-screen">' +
      renderAuthSection(authState) +
      (authState.currentUser && authState.currentUser.role === "admin"
        ? renderAdminSectionEnhanced(authState.adminUsers, authState.selectedAdminUsers)
        : '<section class="feature-card profile-helper-card"><h3>Profil</h3><p class="mode-meta">Po zalogowaniu tutaj zobaczysz swoje konto, zapis punktow, streak i panel administracyjny.</p></section>') +
    '</div>';
    ...
    container.querySelectorAll("[data-admin-kick-room]").forEach(function (button) {
      button.addEventListener("click", function () {
        handlers.onAdminKickRoom(button.dataset.adminKickRoom);
      });
    });
  ...
  function renderMultiplayerLobbySection(container, multiplayerState, currentUser, handlers) {
    container.innerHTML = '<div class="content-grid nav-section-screen">' +
      renderMultiplayerPanel(multiplayerState, currentUser) +
      '<section class="feature-grid">' +
        '<article class="feature-card"><h3>Szybki start</h3><p class="mode-meta">Stworz pokoj albo dolacz po kodzie bez przechodzenia przez inne widoki.</p></article>' +
        '<article class="feature-card"><h3>Realtime</h3><p class="mode-meta">Stan pokoju, timer i wynik sa synchronizowane na zywo przez Socket.IO.</p></article>' +
      '</section>' +
    '</div>';

    container.querySelector("#createRoomButton").addEventListener("click", handlers.onCreateRoom);
    container.querySelector("#joinRoomForm").addEventListener("submit", function (event) {
      event.preventDefault();
      const form = event.currentTarget;
      handlers.onJoinRoom(form.roomCode.value);
    });
  }
  ...
  function scrollGameContainerToAnswerArea() {
    if (!gameContainer) {
      return;
    }

    window.requestAnimationFrame(function () {
      const answerTarget = gameContainer.querySelector("[data-answer-anchor]");
      ...
      const containerWindowTop = Math.max(0, containerPageTop - ((viewportHeight - containerRect.height) / 2));

      window.scrollTo({
        top: containerWindowTop,
        behavior: "smooth"
      });

      const targetHeight = answerTarget.offsetHeight || 0;
      const containerHeight = gameContainer.clientHeight || 0;
      const topOffset = Math.max(
        0,
        answerTarget.offsetTop - ((containerHeight - targetHeight) / 2)
      );
      ...
      const focusTarget = answerTarget.matches("input, button")
        ? answerTarget
        : answerTarget.querySelector("input:not([disabled]), button:not([disabled])");
  ...
  function renderFightGame(container, gameState, handlers, meta) {
    ...
    container.innerHTML = '<div class="content-grid">' +
      ...
      '<section class="classic-panel">' +
        ...
        '</div>' +
      '</section>' +
      '<section class="fight-grid" data-answer-anchor>' + fighterCards + '</section>' +
      resultBlock +
  ...
  function renderLoreGame(container, gameState, handlers, meta) {
    ...
    container.innerHTML = '<div class="content-grid">' +
      ...
      '<section class="classic-panel">' +
        ...
        '<form class="guess-form" id="loreGuessForm" data-answer-anchor>' +
  ...
  function renderDailyGame(container, gameState, handlers, meta) {
    ...
    container.innerHTML = '<div class="content-grid">' +
      ...
      '<section class="classic-panel">' +
        ...
        '<form class="guess-form" id="dailyGuessForm" data-answer-anchor>' +
  ...
  function renderPowerGame(container, gameState, handlers, meta) {
    ...
    container.innerHTML = '<div class="content-grid">' +
      ...
      '<section class="classic-panel">' +
        ...
        '<div class="power-options-grid" data-answer-anchor>' + optionButtons + '</div>' +
        '<form class="guess-form" id="powerGuessForm" data-answer-anchor>' +
  ...
  const initialModeFromUrl = getModeFromUrl();

  const store = createStore({
    activeView: initialModeFromUrl ? "mode" : "menu",
  ...
  if (initialModeFromUrl) {
    store.setState({ activeModeId: initialModeFromUrl });
  }
  ...
  function handleNavigate(sectionId) {
    const nextSection = APP_NAV_ITEMS.some(function (item) {
    ...
    if (nextSection === "multiplayer") {
      syncRoute(null);
      store.setState({ activeSection: "multiplayer" });
      return;
    }

    syncRoute(null);
    store.setState({ activeSection: nextSection });
  }
  ...
  function openMenu() {
    ...
    if (store.getState().activeView === "multiplayer") {
      disconnectMultiplayerSocket();
      store.setState({
        multiplayer: {
          status: "idle",
    ...
    syncRoute(null);
    store.setState({ activeView: "menu", activeModeId: null, activeSection: "game" });
  }
  ...
  async function enterMultiplayerRoom(roomResponse, message) {
    const state = store.getState();

    if (!state.authToken) {
      store.setState({
        authStatus: "error",
        authMessage: "Zaloguj sie, aby korzystac z multiplayera."
      });
      return;
    }
    ...
    try {
      store.setState({
        ...
        multiplayer: Object.assign({}, store.getState().multiplayer, {
          status: roomResponse.room.status || "waiting",
          message: message,
          roomCode: roomResponse.roomCode,
          players: roomResponse.room.players || [],
          me: roomResponse.room.me || null,
          opponent: roomResponse.room.opponent || null,
          question: null,
          hangman: roomResponse.room.hangman || null,
          waitingForOpponent: Boolean(roomResponse.room.waitingForOpponent),
          result: roomResponse.room.result || null,
          roundEndsAt: null,
          secondsLeft: typeof roomResponse.room.secondsLeft === "number" ? roomResponse.room.secondsLeft : 0,
          selectedOptionId: ""
        })
      });

      const socket = await connectMultiplayerSocket(state.authToken);
      socket.emit("multiplayer:join-room", {
        roomCode: roomResponse.roomCode
      });
    } catch (error) {
      store.setState({
  ...
  async function handleCreateRoom() {
    ...
    if (!state.authToken) {
      store.setState({
        authStatus: "error",
        authMessage: "Zaloguj sie, aby stworzyc pokoj multiplayer."
      });
      return;
    }

    try {
      const response = await createMultiplayerRoom(state.authToken);
      await enterMultiplayerRoom(response, "Pokoj utworzony. Udostepnij kod przeciwnikowi.");
    } catch (error) {
      store.setState({
        authStatus: "error",
        authMessage: error.message
  ...
  async function handleJoinRoom(roomCode) {
    ...
    if (!state.authToken) {
      store.setState({
        authStatus: "error",
        authMessage: "Zaloguj sie, aby dolaczyc do pokoju multiplayer."
      });
      return;
    }
    ...
    if (!normalizedCode) {
      store.setState({
        authStatus: "error",
        authMessage: "Podaj kod pokoju."
      });
      return;
    }

    try {
      const response = await joinMultiplayerRoom(state.authToken, normalizedCode);
      await enterMultiplayerRoom(response, "Dolaczono do pokoju " + normalizedCode + ".");
    } catch (error) {
      store.setState({
        authStatus: "error",
        authMessage: error.message
  ...
  function handleMultiplayerAnswer(answerId) {
    const state = store.getState();
    const multiplayer = state.multiplayer || {};
    const hangman = multiplayer.hangman || null;
    const normalizedLetter = String(answerId || "").trim().toUpperCase();
    const usedLetters = hangman && Array.isArray(hangman.usedLetters)
      ? hangman.usedLetters
      : hangman && Array.isArray(hangman.guessedLetters)
        ? hangman.guessedLetters
    ...
    if (usedLetters.indexOf(normalizedLetter) !== -1) {
      return;
    }

    multiplayerSocket.emit("multiplayer:guess-letter", {
  ...
  async function handleAdminKickRoom(userId) {
    const state = store.getState();
    ...
    if (!window.confirm("Na pewno wyrzucic tego gracza z aktywnego pokoju?")) {
      return;
    }

    try {
      await kickUserFromRoomAsAdmin(state.authToken, userId);
      store.setState({
        authStatus: "success",
        authMessage: "Gracz zostal wyrzucony z pokoju."
      });
      refreshAdminUsers();
    } catch (error) {
      store.setState({
  ...
  function openMode(modeId, options) {
    ...
    syncRoute(modeId);
    store.setState({ activeView: "mode", activeModeId: modeId, activeSection: "game" });
  }
  ...
  function updateApp(state) {
    ...
    const isMenuScreen = state.activeSection === "game" && state.activeView === "menu";
    menuView.hidden = !isMenuScreen;
    if (gameLayout) {
      gameLayout.classList.toggle("is-single-view", !isMenuScreen);
    }

    if (state.activeSection === "home") {
      applySeo(null);
      homeButton.hidden = true;
      renderHomeSection(gameContainer, {
    ...
    if (state.activeSection === "ranking") {
      applySeo(null);
      homeButton.hidden = true;
      renderRankingSection(gameContainer, state.leaderboard, {
        onStart: handleStartGame
      });
      return;
    }
    ...
    if (state.activeSection === "profile") {
      applySeo(null);
      homeButton.hidden = true;
      renderProfileSection(gameContainer, {
        currentUser: state.currentUser,
        view: state.authView,
        ...
        onAdminKickRoom: handleAdminKickRoom,
    ...
    if (state.activeSection === "multiplayer") {
      applySeo(null);
      homeButton.hidden = true;
      renderMultiplayerLobbySection(gameContainer, state.multiplayer, state.currentUser, {
    ...
    if (state.activeSection === "game" && state.activeView === "menu") {
      applySeo(null);
      homeButton.hidden = true;
      renderGameSection(gameContainer, state.leaderboard, state.progress, {
        onStart: handleStartGame
      }, state.dataStatus);
      return;
    }
